...
const RenderSteps = () => {
  ...
  return (
    <>
      <div className="relative mb-2 flex w-[17rem] md:w-full justify-center">
        {steps.map((item,i) => (
          <>
            <div 
            key={i}
            className="flex flex-col items-center">
              <button
                ...
                className={`grid cursor-pointer aspect-square w-[34px] place-items-center rounded-full border-[1px] ${
                  step === item.id
                    ? "border-yellow-50 bg-yellow-900 text-yellow-300"
      ...
      <div className="relative mb-16 flex w-full select-none justify-between">
        {
            steps.map((item,index)=>(
                <>
                <div className="flex md:min-w-[130px] flex-col items-center gap-y-2" key={index}>
                    <p className={`text-sm  ${
                        step>=item.id
                        ?"text-white"
                        :"text-richblack-500"
